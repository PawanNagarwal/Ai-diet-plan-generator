...
Guidelines:
1. Use household measurements (bowls, plates, cups, pieces, slices, etc.)
...
def display_form(generator, is_edit=False):
    ...
    with st.form("diet_form", clear_on_submit=not is_edit):
        ...
        # Calculate and display sleep schedule info
        if sleep_time_input and wake_time_input:
            sleep_dt = datetime.combine(datetime.today(), sleep_time_input)
            wake_dt = datetime.combine(datetime.today(), wake_time_input)
            
            if sleep_time_input > wake_time_input:
                sleep_dt += timedelta(days=1)
            
            actual_sleep_hours = (sleep_dt - wake_dt).total_seconds() / 3600
            awake_hours = 24 - actual_sleep_hours
            
            # Calculate meal times for preview
            meal_timing_calc = MealTimingCalculator()
            preview_meal_times = meal_timing_calc.calculate_meal_times(
                sleep_time_input, wake_time_input, meals_per_day
            )
            
            # st.markdown(f"""
            # <div class="sleep-schedule-card">
            #     <h4 style="margin: 0 0 1rem 0;">📊 Your Schedule Preview</h4>
            #     <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
            #         <div><strong>Sleep Duration:</strong> {actual_sleep_hours:.1f} hours</div>
            #         <div><strong>Awake Duration:</strong> {awake_hours:.1f} hours</div>
            #     </div>
            #     <div style="margin-top: 1rem;">
            #         <strong>🍽️ Suggested Meal Times:</strong><br>
            #         {' • '.join([f"{meal}: {time}" for meal, time in preview_meal_times.items()])}
            #     </div>
            # </div>
            # """, unsafe_allow_html=True)
...
def display_results():
    ...
    with col3:
        if st.button("🆕 New Plan", use_container_width=True):
            # Clear session state
            for key in ['form_data', 'diet_plan', 'days_data', 'meal_times']:
